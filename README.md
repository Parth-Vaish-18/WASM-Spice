...
WASM-Spice provides a highly responsive, drag-and-drop workspace where users can build, wire, and test analog circuits intuitively. 

### Component Library
The simulator natively supports a wide array of linear and active components:
* **Passive Elements:** Resistors, Capacitors, and Inductors.
* **Independent Sources:** DC Voltage and DC Current sources.
* **Dependent Sources:** Full mathematical implementation of Voltage-Controlled Voltage Sources (VCVS) and Voltage-Controlled Current Sources (VCCS) for simulating complex active components like operational amplifiers.
* **Ground Nodes:** Essential reference nodes for MNA calculations.
...
* **Interactive React Flow Canvas:** A highly responsive, node-based UI. Users can drag, drop, and wire components naturally. The UI dynamically generates a netlist as you draw, creating complex schematics in seconds.
...
* **Save/Load Workspaces:** Export your complex schematics as `.json` files and load them later to pick up exactly where you left off.

---

## ⚡ The Engineering Architecture

WASM-Spice is architected to separate heavy computation from the visual rendering layer:
...
* **The C++ Engine (Math Core):** 1. Parses the incoming netlist. 
  2. Constructs the G (conductance), B, C, and D matrices based on Kirchhoff's Current and Voltage Laws. 
  3. Solves the linear system `Ax = z` at 500+ iterations per second using optimized numerical methods. 
